...
pub(crate) fn cmp_impl(d1: &Decimal, d2: &Decimal) -> Ordering {
    // Same sign and scale: the mantissas decide.
    if d1.flags() == d2.flags() {
        let ord = mantissa(d1).cmp(&mantissa(d2));
        return if d1.is_sign_negative() { ord.reverse() } else { ord };
    }
...
#[inline(always)]
fn mantissa(d: &Decimal) -> u128 {
    let [lo, mid, hi] = d.mantissa_array3();
