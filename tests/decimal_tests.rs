...
#[test]
fn it_converts_to_f32() {
    let tests = &[
        ("5", 5f32),
        ("-5", -5f32),
        ("0.1", 0.1f32),
        ("0.0", 0f32),
        ("0.0000000000025", 0.25e-11f32),
        ("1.59283191", 1.59283191_f32),
        ("2.2238", 2.2238_f32),
        ("2.2238123", 2.2238123_f32),
        ("22238", 22238_f32),
        ("1000000", 1000000_f32),
        ("1000000.000000000000000000", 1000000_f32),
        ("79228162514264337593543950335", 79228162514264337593543950335_f32),
        ("-79228162514264337593543950335", -79228162514264337593543950335_f32),
    ];
    for &(value, expected) in tests {
        let value = Decimal::from_str(value).unwrap();
        assert_eq!(expected, value.as_f32());
        assert_eq!(Some(expected), value.to_f32());
    }
}
